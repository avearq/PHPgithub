...
class OrderController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Order']))
		{
			...
			$model->CusID=Yii::app()->session['CusID'];
			if($model->save())
				...
				if(isset($session['intLine'] ) && ($session['intLine'] > 0))
				{
					...
					for($i=0;$i<=(int)$session["intLine"];$i++)
					{
						if($arrProductID[$i] != ""){
							...
							$modelOrderDetail->Order_ID=$model->Order_ID;
							$modelOrderDetail->Product_ID=$arrProductID[$i];
							$modelOrderDetail->Product_Price=$arrProductPrice[$i];
							$modelOrderDetail->Product_Qty=$arrProductQty[$i];
